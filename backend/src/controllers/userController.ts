...
export async function createUser(req: Request, res: Response) {
  try {
    ...
    if (existingUser) {
      res.status(400).json({ message: USERNAME_TAKEN_MESSAGE });
      return;
    }
    ...
    if (existingEmail) {
      res.status(400).json({ message: EMAIL_TAKEN_MESSAGE });
      return;
    }
...
/**
 * Allows users to update their profile information, including username, email, and password.
 *
...
export const updateUser = async (req: Request, res: Response) => {
  const { id, username, email, newPassword } = req.body;
  ...
  try {
    ...
    if (newPassword) {
      if (isPasswordValid(newPassword)) {
        res.status(400).json({ message: PASSWORD_ERROR_MESSAGE });
        return;
      }
